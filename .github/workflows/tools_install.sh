...
sudo apt-get update
sudo apt-get install libtinfo5 libtinfo6 dc libxml2-utils graphviz libffi7
...
if [[ "x${LOAD_FSL}x" == "x1x" ]]; then
    source $GITHUB_WORKSPACE/aa_tools/toolboxes/installation_scripts/install_fsl.sh $TOOLDIR 6.0.6.2 0 $TOOLDIR/config/fsl_bash.sh
fi
